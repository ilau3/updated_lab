...
Python wrapper script for PICam.dll to control PIXIS cam.
Based on work by Daniel R. Dietze
Adapted for Python 3 by Lorenz Drescher <lbdrescher@berkeley.edy> 
...
class picam():
    ...
    def loadLibrary(self, pathToLib=""):
        ...
        :returns: string of the library version.
        """
        if pathToLib == "":
    ...
    def getAvailableCameras(self):
        ...
        if id_count.value < 1:
            ...
            msg_string = '  Model is ' + PicamModelLookup[model_ID.model]
            msg_string += '\n  Computer interface is ' + PicamComputerInterfaceLookup[model_ID.computer_interface]
            msg_string += '\n  Sensor_name is ' + model_ID.sensor_name
            msg_string += '\n  Serial number is'+ model_ID.serial_number
            msg_string += '\n'
            logger.info(msg_string)
        else:
            for i in range(id_count.value):
                msg_string = '  Model is ' + PicamModelLookup[self.camIDs[i].model]
                msg_string += '\n  Computer interface is ' + PicamComputerInterfaceLookup[self.camIDs[i].computer_interface]
                msg_string += '\n  Sensor_name is ' + str(self.camIDs[i].sensor_name)
                msg_string += '\n  Serial number is' + str(self.camIDs[i].serial_number)
                msg_string += '\n'
    ...
    def getParameter(self, name):
        ...
        if not exists.value:
            logger.warning("Ignoring parameter "+ name + ".  Parameter does not exist for current camera!")
            return
        ...
        if type.value not in PicamValueTypeLookup:
            logger.warning("Not a valid parameter type enumeration: " + type.value + ". Ignoring parameter "+ name)
            return 0
    ...
    def setParameter(self, name, value):
        ...
        if PicamValueAccessLookup[access.value] not in ["ReadWrite", "ReadWriteTrivial"]:
            logger.warning("Ignoring parameter " + name + ". Not allowed to overwrite parameter!")
            return
        if PicamValueAccessLookup[access.value] == "ReadWriteTrivial":
            logger.warning("Parameter" + name + " allows only one value!")
        ...
        if type.value not in PicamValueTypeLookup:
            logger.warning("Ignoring parameter " + name + ". Not a valid parameter type:" + type.value)
            return
        ...
        if self.err != PicamError["None"]:
            logger.warning("Ignoring parameter {}. Could not change parameter. Keeping previous value: {}".format(name,self.getParameter(name)))
    ...
    def sendConfiguration(self):
        ...
        if failedCount.value > 0:
            for i in range(failedCount.value):
                logger.error("Could not set parameter " + PicamParameterLookup[failed[i]])
    ...
    def getROI(self):
        """ Return a signle region of interest (ROI) as a tuple. """
        r = self.getParameter("Rois")
        try:
            rr = r.roi_array[0]
        except IndexError:
            return None
        return rr.x, rr.width, rr.y, rr.height, rr.x_binning, rr.y_binning
    ...
    # N = number of frames
    # timeout = max wait time between frames in ms or -1 for no timeout
    def readNFrames(self, N=1, timeout=-1):
        ...
        if running.value:
            logger.error("Acquisition still running")
            return []
    ...
    def startAcquisition(self):
        return self.lib.Picam_StartAcquisition(self.cam)

    def stopAcquisition(self):
        self.lib.Picam_StopAcquisition(self.cam)

    def waitForFrame(self,timeout=0,frames=1):
        available = PicamAvailableData()
        status = PicamAcquisitionStatus()

        err = self.lib.Picam_WaitForAcquisitionUpdate(self.cam,piint(timeout),ptr(available), ptr(status))

        # return data as numpy array
        if available.readout_count >= frames:
            if len(self.ROIS) == 1:
                return err, self.getBuffer(available.initial_readout, available.readout_count)[0:frames]
            else:
                return err, self.getBuffer(available.initial_readout, available.readout_count)[:][0:frames]
        return err, None
    ...
    def getBuffer(self, address, size):
        ...
        if len(self.ROIS) == 1:
            return [data.reshape(size * frames, self.ROIS[0][1], self.ROIS[0][0])] #I switched these and now it seems to work
    ...
    def _allocate_buffer(self,nframes=100):
        # Calculate buffer size
        readoutstride = self.getParameter("ReadoutStride")
        framestride = self.getParameter("FrameStride")
        framesPerRead = self.getParameter("FramesPerReadout")
        nreadouts=(nframes-1)//framesPerRead+1
        # Deallocate old buffer
        self._deallocate_buffer()
        # Allocate new
        self._buffer=ctypes.create_string_buffer(readoutstride*nreadouts)
        self.lib.PicamAdvanced_SetAcquisitionBuffer(self.cam,ctypes.addressof(self._buffer),readoutstride*nreadouts)

    def _deallocate_buffer(self):
        self.lib.PicamAdvanced_SetAcquisitionBuffer(self.cam,0,0)
        self._buffer=None        
...
if __name__ == '__main__':
    ...
    # cool down CCD
    cam.setParameter("SensorTemperatureSetPoint", 20)
    ...
    # custom chip settings
    cam.setROI(0, 1340, 1, 0, 400, 1) #currently no binning
    cam.setParameter("ActiveWidth", 1340)
    cam.setParameter("ActiveHeight", 400)
    cam.setParameter("ActiveLeftMargin", 0)
    cam.setParameter("ActiveRightMargin", 0)
    cam.setParameter("ActiveTopMargin", 8)
    cam.setParameter("ActiveBottomMargin", 8)
    ...
    #cam.setParameter("CleanSerialRegister", False)
    cam.setParameter("CleanCycleCount", 1)
    cam.setParameter("CleanCycleHeight", 8)
    cam.setParameter("CleanUntilTrigger", True)
